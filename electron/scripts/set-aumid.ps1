...
# Implementation note
# -------------------
# All COM interop happens inside the C# Helper.SetAumid method.
# Casting (IPersistFile)link in C# triggers a QueryInterface under
# the hood; PowerShell's own cast operator does NOT do QI on COM
# objects, so trying to cast in script fails with "Cannot convert
# CShellLink to IPersistFile". Keeping it in C# avoids that trap.
#
# Usage
# -----
...
namespace NexusLauncher {
  ...
  public static class Helper {
    ...
    private static readonly Guid FMTID_AppUserModel =
    ...
    public static void SetAumid(string lnkPath, string aumid) {
      ...
      try {
        // Both casts trigger runtime QueryInterface on the underlying
        // IUnknown — that's the magic that PowerShell's [Type] cast
        // can't do, which is why this helper lives in C# code.
...
[NexusLauncher.Helper]::SetAumid($LinkPath, $Aumid)
